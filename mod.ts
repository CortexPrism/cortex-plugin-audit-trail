...
export async function onLoad(ctx: PluginContext): Promise<void> {
  ...
  console.log(`[cortex-plugin-audit-trail] Loaded (format: ${defaultFormat}, interval: ${defaultInterval})`);
}
...
const scheduledExports = new Map<string, { destination: string; interval: string; createdAt: string }>();
...
const auditExportTool: Tool = {
  definition: {
    ...
    params: [
      ...
      { name: "filter", type: "string", description: "JSON query filter for log selection", required: false },
    ],
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (!destination) {
        return { toolName: "audit_export", success: false, output: "", error: "destination is required", durationMs: Date.now() - start };
      }

      const validDestinations = ["splunk", "elastic", "datadog", "file", "s3"];
      if (!validDestinations.includes(destination)) {
        return { toolName: "audit_export", success: false, output: "", error: `Invalid destination. Valid: ${validDestinations.join(", ")}`, durationMs: Date.now() - start };
      }
      ...
      return { toolName: "audit_export", success: false, output: "", error: `Export failed: ${error instanceof Error ? error.message : String(error)}`, durationMs: Date.now() - start };
    }
  },
};
...
const auditScheduleTool: Tool = {
  definition: {
    ...
    params: [
      ...
      { name: "destination", type: "string", description: "Export destination for the schedule", required: false },
      { name: "interval", type: "string", description: "Recurring interval", required: false },
      { name: "schedule_id", type: "string", description: "Schedule ID for delete action", required: false },
    ],
    capabilities: [],
  },
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (action === "create") {
        if (!destination) {
          return { toolName: "audit_schedule", success: false, output: "", error: "destination is required for create", durationMs: Date.now() - start };
        }
      ...
      if (action === "delete") {
        if (!scheduleId) {
          return { toolName: "audit_schedule", success: false, output: "", error: "schedule_id is required for delete", durationMs: Date.now() - start };
        }
      ...
      return { toolName: "audit_schedule", success: false, output: "", error: `Unknown action: ${action}`, durationMs: Date.now() - start };
    } catch (error) {
      return { toolName: "audit_schedule", success: false, output: "", error: `Schedule operation failed: ${error instanceof Error ? error.message : String(error)}`, durationMs: Date.now() - start };
    }
  },
};

const SENSITIVE_FIELDS = ["password", "secret", "token", "apiKey", "api_key", "credential", "private_key", "authorization"];
...
const auditRedactTool: Tool = {
  definition: {
    ...
    params: [
      ...
      { name: "redaction_rules", type: "string", description: "JSON array of field paths to redact", required: false },
    ],
    capabilities: [],
  },
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (!dataStr) {
        return { toolName: "audit_redact", success: false, output: "", error: "data is required", durationMs: Date.now() - start };
      }
      ...
      try {
        ...
        return { toolName: "audit_redact", success: false, output: "", error: "data must be valid JSON", durationMs: Date.now() - start };
      }
        ...
        for (const [key, value] of Object.entries(obj)) {
          ...
          if (isSensitive) {
            ...
            result[key] = value.map((v) =>
              typeof v === "object" && v !== null ? redact(v as Record<string, unknown>, [...path, key]) : v
            );
          } else {
            result[key] = value;
          }
      ...
      return {
        ...
        success: true,
        output: JSON.stringify({ redacted, fieldsRedacted: SENSITIVE_FIELDS.length + customRules.length }, null, 2),
        durationMs: Date.now() - start,
      };
    } catch (error) {
      return { toolName: "audit_redact", success: false, output: "", error: `Redaction failed: ${error instanceof Error ? error.message : String(error)}`, durationMs: Date.now() - start };
    }
  },
};

const complianceFormatters: Record<string, (event: Record<string, unknown>) => Record<string, unknown>> = {
  soc2: (event) => ({
    ...event,
...
const auditComplianceFormatTool: Tool = {
  definition: {
    ...
    params: [
      ...
      { name: "framework", type: "string", description: "Target compliance framework", required: true },
    ],
    capabilities: [],
  },
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (!dataStr || !framework) {
        return { toolName: "audit_compliance_format", success: false, output: "", error: "data and framework are required", durationMs: Date.now() - start };
      }
      ...
      try {
        ...
        return { toolName: "audit_compliance_format", success: false, output: "", error: "data must be valid JSON", durationMs: Date.now() - start };
      }

      const formatter = complianceFormatters[framework];
      if (!formatter) {
        return { toolName: "audit_compliance_format", success: false, output: "", error: `Unknown framework: ${framework}. Valid: soc2, hipaa, gdpr, pci_dss`, durationMs: Date.now() - start };
      }
      ...
      return {
        ...
        success: true,
        output: JSON.stringify({ framework, originalEvent: data, complianceFormatted: formatted }, null, 2),
        durationMs: Date.now() - start,
      };
    } catch (error) {
      return { toolName: "audit_compliance_format", success: false, output: "", error: `Format failed: ${error instanceof Error ? error.message : String(error)}`, durationMs: Date.now() - start };
    }
  },
};
...
const auditStatsTool: Tool = {
  definition: {
    ...
    params: [
      { name: "time_range", type: "string", description: "Time range for statistics", required: false },
    ],
    capabilities: [],
  },
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      return { toolName: "audit_stats", success: false, output: "", error: `Stats failed: ${error instanceof Error ? error.message : String(error)}`, durationMs: Date.now() - start };
    }
  },
};
